...
export default class Turno extends React.Component {
  ...
  dispatchToken(self) {
    this.props.requestUpdateDispatched(self.props.id, { dispatched: true, dispatchAt: new Date() });
  }//end dispatchToken
                ...
                this.props.tokens.map((token, index) => {
                  return (
                    <li key={ index } className={ index === 0 ? "" : "disable-token"}>
                      <Token id={ token.id } dispatched={ true } lift={ this.addChildren.bind(this, token.name) } name={ token.name } onCancel={ this.cancelToken.bind(this) } onCall={ this.callUser.bind(this) } onDispatch={ this.dispatchToken.bind(this) }/>
                    </li>
                  )
                })
              }
